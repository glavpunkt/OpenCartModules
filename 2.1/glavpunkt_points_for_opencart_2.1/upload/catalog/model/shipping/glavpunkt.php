...
class ModelShippingGlavpunkt extends Model
{
    function getQuote($address)
    {
        ...
        } elseif ($this->config->get('glavpunkt_weight') > 0) {
            $weight = $this->config->get('glavpunkt_weight');
        } else {
            $weight = 1;
        }
        ...
        if ($status) {
              ...
              $quote_text .= '
                   ...
                   $("input:radio[value=\'glavpunkt.glavpunkt\']").prop("checked", true);
                    ...
                    $.ajax({
                      ...
                      type: \'post\',
                      data: {price:tarif, type:\'Главпункт Пункты Выдачи по РФ\', info:punktInfo.name, address:punktInfo.address, phone:punktInfo.phone, work_time:punktInfo.work_time, city_to:punktInfo.city},
                      dataType: \'html\',
            ...
            if ($this->config->get('glavpunktpoints_simple_status') == 1) {
                ...
                $quote_text .= '$(\'#glavpunkt_content\').html(html);';
            }
            $quote_text .= '}
                    });
