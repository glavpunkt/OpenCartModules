...
class ControllerExtensionModuleGlavpunktorders extends Controller
{
    ...
    public function index()
    {
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if (isset($this->request->post['selected']) && count($this->request->post['selected'])) {
                ...
                $invoiceInfo = [
                    ...
                    'token' => $this->config->get('glavpunktorders_token'),
                    // список заказов
            ...
            $this->response->redirect(
                $this->url->link('extension/module/glavpunktorders', 'token=' . $this->session->data['token'], 'SSL')
            );
        }
        ...
        $this->data['form_login_value'] = $this->config->get('glavpunktorders_login') !== ''
            ? $this->config->get('glavpunktorders_login')
            : '';
        ...
        $this->data['form_token_value'] = $this->config->get('glavpunktorders_token') !== ''
            ? $this->config->get('glavpunktorders_token')
            : '';
        ...
        $this->data['breadcrumbs'][] = [
            ...
            'href' => $this->url->link('extension/module/store', 'token=' . $this->session->data['token'], 'SSL')
        ];
        ...
        $this->response->setOutput($this->load->view('extension/module/glavpunktorders.tpl', $this->data));
    }
     ...
     *
     * заказ не сохраняет идентификатор города, поэтому мы сравниваем id пункта со списком пунктов выдачи по РФ
     *
    ...
    private function getTrackingUrl($sku)
    {
        ...
        curl_setopt_array($curl, [
            ...
            CURLOPT_USERAGENT => 'opencart-2.3'
        ]);
        $answer = curl_exec($curl);
        curl_close($curl);
        $answer = json_decode($answer, true);
    ...
    private function ComposeOrder($info, $items, $punktId = null)
    {
        ...
        $delivPrice = $info['total'] - $productsPrice;
        if ($delivPrice > 0) {
            $parts[] = [
    ...
    private function CreateInvoice($request)
    {
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, "opencart-2.3");
