...
/**
 * Контроллер пунктов выдачи Главпункт в администраторской панели
 *
 * Class ControllerExtensionShippingGlavpunkt
 */
class ControllerExtensionShippingGlavpunkt extends Controller
{
    ...
    public function index()
    {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/shipping/glavpunkt', 'token=' . $this->session->data['token'], true)
        );

        $data['action'] = HTTPS_SERVER .
            'index.php?route=extension/shipping/glavpunkt&token=' .
        ...
        if (isset($this->request->post['glavpunkt_sort_order'])) {
            $data['glavpunkt_sort_order'] = $this->request->post['glavpunkt_sort_order'];
        } else {
            $data['glavpunkt_sort_order'] = $this->config->get('glavpunkt_sort_order');
        }

        if (isset($this->request->post['glavpunkt_home_city'])) {
            $data['glavpunkt_home_city'] = $this->request->post['glavpunkt_home_city'];
        } else {
            $data['glavpunkt_home_city'] = $this->config->get('glavpunkt_home_city');
        }
        if (isset($this->request->post['glavpunkt_weight'])) {
            $data['glavpunkt_weight'] = $this->request->post['glavpunkt_weight'];
        } else {
            $data['glavpunkt_weight'] = $this->config->get('glavpunkt_weight');
        }
        if (isset($this->request->post['glavpunkt_tarif_edit_code'])) {
            $data['glavpunkt_tarif_edit_code'] = $this->request->post['glavpunkt_tarif_edit_code'];
        } else {
            $data['glavpunkt_tarif_edit_code'] = $this->config->get('glavpunkt_tarif_edit_code');
        }

        if (isset($this->request->post['glavpunkt_widget_data'])) {
            $data['glavpunkt_widget_data'] = $this->request->post['glavpunkt_widget_data'];
        } else {
            $data['glavpunkt_widget_data'] = $this->config->get('glavpunkt_widget_data');
        }

        if (isset($this->request->post['glavpunkt_price_spb'])) {
            $data['glavpunkt_price_spb'] = $this->request->post['glavpunkt_price_spb'];
        } else {
            $data['glavpunkt_price_spb'] = $this->config->get('glavpunkt_price_spb');
        }

        if (isset($this->request->post['glavpunkt_price_msk'])) {
            $data['glavpunkt_price_msk'] = $this->request->post['glavpunkt_price_msk'];
        } else {
            $data['glavpunkt_price_msk'] = $this->config->get('glavpunkt_price_msk');
        }
    ...
    {
        if (!$this->user->hasPermission('modify', 'extension/shipping/glavpunkt')) {
