...
class ControllerModuleGlavpunktorders extends Controller
{
        ...
        if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
            ...
            if (isset($this->request->post['selected']) && count($this->request->post['selected'])) {
                ...
                foreach ($this->request->post['selected'] as $orderId) {
                    ...
                    if ($order_info['shipping_code'] === 'glavpunkt.pickup') {
    ...
    private function getOrdersList()
    {
        ...
        foreach ($results as $result) {
            // получение заказа более подробно, чтобы получить код доставки
            // в версиях 2.1 это выводилось сразу, а тут необходимо искать
            ...
            switch ($order_info['shipping_code']) {
                case "glavpunkt.pickup":
                    $shippingTitle = 'Пункты выдачи Главпункт';
                    break;
                case "glavpunkt.courier":
        ...
        if ($this->config->get('config_limit_admin') > 0) {
            $numPage = ceil($order_total / $this->config->get('config_limit_admin'));
        } else {
            $numPage = 1;
        }
    ...
    private function CreateInvoice($request)
    {
        ...
        curl_setopt($curl, CURLOPT_USERAGENT, "opencart-v1.5");
