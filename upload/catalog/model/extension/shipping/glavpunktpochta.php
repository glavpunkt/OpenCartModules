...
class ModelExtensionShippingGlavpunktpochta extends Model
{
    function getQuote($address)
    {
        ...
        if ($status) {
            ...
            if ($this->config->get('shipping_glavpunktpochta_tarif_edit_code') !== null) {
                $order = array("&nbsp;", "&lt;", "&gt;", "&amp;", "&quot;", "&apos;");
                $replace = array(" ", "<", ">", '"', "'");
                $userSettingsPochta = str_replace($order, $replace, $this->config->get('shipping_glavpunktpochta_tarif_edit_code'));
            } else {
                $userSettingsPochta = "";
            }
            // массив параметров
