...
class ModelExtensionShippingGlavpunkt extends Model
{
    function getQuote($address)
    {
        ...
        if ($this->config->get('shipping_glavpunkt_status') == 1) {
            $status = true;
        } else {
            $status = false;
        }

        if ($this->weight->convert(
                $this->cart->getWeight(),
        ...
        if (isset($address['city']) && $address['city'] != '') {
            ...
            $city = $address['zone'];
        } elseif ($this->config->get('shipping_glavpunkt_home_city') != null) {
            $city = $this->config->get('shipping_glavpunkt_home_city');
        } else {
        ...
        if ($status) {
            ...
            if ($this->config->get('shipping_glavpunkt_tarif_edit_code')) {
                $order = array("&nbsp;", "&lt;", "&gt;", "&amp;", "&quot;", "&apos;");
                $replace = array(" ", "<", ">", '"', "'");
                $userSettings = str_replace(
                    $order,
                    $replace,
                    $this->config->get('shipping_glavpunkt_tarif_edit_code')
                );
            } else {
                $userSettings = '';
            }
            $dataForWidget = "{'defaultCity': '" . $city .
                "'," . $this->config->get('shipping_glavpunkt_widget_data') . "}";
            $textDescription = $this->language->get('text_description');

            // приводим к нормальному виду :)
            // выводим скрипт и стиль для карты

            $quote_text = <<<EOD
                <a 
                    id="glavpunkt_open_map"  
                    href="#" 
                    onclick="glavpunkt.openMap(selectPunkt, $dataForWidget); return false;" 
                ...
                <script type="text/javascript">
                    ...
                    function selectPunkt(punktInfo) {
                        ...
                        $.getJSON( "//glavpunkt.ru/api/get_tarif", {
                            'serv': 'выдача',
                            'cityFrom': '{$this->config->get('shipping_glavpunkt_home_city')}',
